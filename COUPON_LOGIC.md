...
// 2. Происходит навигация на релевантную страницу
...
1. **Страница оформления** (`checkout.html`)
   ...
   - Пользователь включает нужные купоны
   - Рассчитывается скидка в реальном времени

2. **Оформление пакета**
   - Активные купоны помечаются как `used: true`
   - Создается запись об инвестиции
   - Обновляется интерфейс

3. **Результат**
   - Купоны исчезают из активных
   - Появляются в истории использованных
...
### **Сценарий 3: Просмотр истории купонов**
...
### **5. Отображение статусов купонов (full-coupon-card.js)**
